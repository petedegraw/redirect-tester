...
fs.readdir(process.env.redirects_path, function(err, files) {
    ...
    inquirer
        ...
        .then(answers => {
            ...
            const report_file = process.env.reports_path + '/' + answers.redirect_file.split('.')[0] + '-' + answers.environment + '-' + d_file + '.pdf';
            
            doc.pipe(fs.createWriteStream(report_file));
            
            doc.font('Helvetica')
                .fontSize(14)
                .text('Redirect Tests: ' + answers.redirect_file.split('.')[0].toUpperCase() + ' — Production')
                .moveDown(.5);
